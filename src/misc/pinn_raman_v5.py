...
def physics_loss(pred, true, pred_aug, zero_n, max_n, pw,
                 lw_mse=LW_MSE, lw_nn=LW_NONNEG, lw_mass=LW_MASS, lw_inv=LW_INV):
    """Compute physics-informed loss. pw = physics ramp weight [0..1]."""
    ...
    inv    = F.mse_loss(pred_aug, pred.detach()) * pw if pred_aug is not None else 0.0

    return lw_mse * mse + lw_nn * nonneg + lw_mass * mass + lw_inv * inv
...
def maml_inner_loop(model: MetaModel, Z_sup: torch.Tensor, y_sup: torch.Tensor,
    ...
    for _ in range(inner_steps):
        ...
        loss  = physics_loss(pred, y_sup, None, zero_n, max_n, pw=0.0)  # MSE only in inner
        fast_opt.zero_grad()
        loss.backward()
        fast_opt.step()
...
def meta_train(model: MetaModel,
    ...
    2. Sample K_SUPPORT + K_QUERY examples  (stratified by concentration quartile)
    3. Inner loop: adapt on support set
    4. Outer loop: compute query loss on adapted model, update original model
    """
    ...
    all_y    = np.concatenate([y for _, y in device_data if y is not None], axis=0)
    ...
    for epoch in range(1, META_EPOCHS + 1):
        ...
        for _ in range(META_TASKS_PER_EPOCH):
            # Sample a device task
            dev_idx  = rng.randint(0, n_devices)
            X_dev, y_dev = device_data[dev_idx]
            if len(X_dev) < K_SUPPORT + K_QUERY:
                continue

            # Project to PCA space and normalise targets
            Z_dev    = pca.transform(X_dev).astype(np.float32)
            ...
            loss = physics_loss(pred_qry, y_qry, pred_aug, zero_n, max_n, pw=pw * 0.5)
...
def fine_tune_once(meta_model: MetaModel,
    ...
    for epoch in range(1, FT_EPOCHS + 1):
        ...
        for z_b, y_b in dl:
            ...
            loss = physics_loss(pred, y_b, pred_aug, zero_n, max_n, pw)
            opt.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
...
def main():
    ...
    if use_meta:
        meta_model = meta_train(meta_model, device_z, pca, device)
