...
class PhysicsInformedLoss(nn.Module):
    ...
    def forward(self, pred_c, true_c, x_in, x_recon, pure_spec,
        ...
        if pw <= 0:
            ...
            losses["nonneg_c"] = F.relu(self.conc_zero - pred_c).pow(2).mean() * pw
            ...
            losses["mass"]   = F.relu(pred_c - self.conc_max).pow(2).mean() * pw
...
def train_cv(X_pp, y_train, X_test_pp, wns, device):
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X_pp)):
        ...
        # Freeze pure_spectra until PHYS_START — prevents Beer-Lambert
        # from destabilising training before regression head is calibrated
        model.pure_spectra.requires_grad_(False)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            # Unfreeze pure_spectra when physics losses activate
            if epoch == PHYS_START and not model.pure_spectra.requires_grad:
                model.pure_spectra.requires_grad_(True)
                # Add it to the optimiser
                optimizer.add_param_group(
                    {"params": [model.pure_spectra], "lr": LR * 0.1}
                )
