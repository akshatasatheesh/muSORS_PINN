...
# ── Vanilla NN (no physics loss) for PINN comparison ──────────────────────────
def fine_tune_once_vanilla(meta_model, Z_tr, y_tr_raw, Z_val, y_val_raw, device, tw, seed=42):
    """
    Vanilla NN baseline: same encoder/head, same target scaling, same feature-space augmentation,
    but WITHOUT any physics modules or physics losses.
    """
    rng = np.random.RandomState(seed)
    model = copy.deepcopy(meta_model).to(device)
    ...
    for ep in range(FT_EPOCHS):
        ...
        for Zb, yb in dl:
            ...
            L_data = weighted_mse(pred, yb, tw_t)
            L_aug = LW_INV * F.mse_loss(pred, pred_a)  # keep same invariance regularizer for fairness
        ...
        if (ep + 1) % 5 == 0:
            ...
            if r2 > best_r2:
                best_r2, best_sd, best_ts, wait = r2, copy.deepcopy(model.state_dict()), tscaler, 0
            else:
                wait += 5
            if wait >= PATIENCE:
                break
...
def cv_fine_tune_vanilla_nn(meta_model, Z_train, y_train, Z_test, device, tw):
    """
    Vanilla NN baseline CV: matches cv_fine_tune_nn() structure but without physics.
    Produces OOF and test preds for direct comparison with PINN.
    """
    kf = KFold(N_FOLDS, shuffle=True, random_state=SEED)
    oof = np.zeros_like(y_train, dtype=np.float32)
    test_preds = np.zeros((Z_test.shape[0], N_OUT), dtype=np.float32)

    print(f"\n{'='*70}\n  Vanilla NN Fine-tuning ({N_FOLDS}-fold x {N_RESTARTS} restarts)\n{'='*70}")
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(Z_train)):
        ...
        for restart in range(N_RESTARTS):
            m, vr2, ts = fine_tune_once_vanilla(
                meta_model, Z_tr, y_tr, Z_val, y_val, device, tw, seed=SEED + fold * 100 + restart
            )
            if vr2 > best_r2:
        ...
        r2s = [r2_score(y_val[:, i], oof[val_idx, i]) for i in range(N_OUT)]
...
def main():
    ...
    # ── 6b. Vanilla NN (baseline) for science-fair comparison ─────────────────
    vanilla_oof, vanilla_test = cv_fine_tune_vanilla_nn(
        meta_model, Z_train_nn, y_train, Z_test_nn, device, tw
    )
    vanilla_r2s = [r2_score(y_train[:,i], vanilla_oof[:,i]) for i in range(N_OUT)]
    vanilla_avg = np.mean(vanilla_r2s)
    print(f"\n  Vanilla NN OOF R²: " + "  ".join(f"{n}={v:.4f}" for n,v in zip(ANALYTE_NAMES, vanilla_r2s)) + f"  Avg={vanilla_avg:.4f}")

    print(f"\n{'='*70}\n  PINN vs Vanilla NN (OOF R²)\n{'='*70}")
    for i,nm in enumerate(ANALYTE_NAMES):
        print(f"  {nm:<7}  Vanilla={vanilla_r2s[i]:.4f}   PINN={nn_r2s[i]:.4f}   Δ={(nn_r2s[i]-vanilla_r2s[i]):+.4f}")
    print(f"  Overall  Vanilla={vanilla_avg:.4f}   PINN={nn_avg:.4f}   Δ={(nn_avg-vanilla_avg):+.4f}")
    ...
    save_submission(vanilla_test, y_train, "v14_vanilla_nn_raw", OUT_DIR)
    ...
    if win_test_preds is not None and win_test_preds.shape[0] == stacked_test.shape[0]:
        ...
        for src_name, src in [('stack',stacked_test),('hgb',hgb_test),('ensemble',ensemble_test),('nn',nn_test),('vanilla_nn',vanilla_test)]:
